...
class Collection_biovision extends CI_Controller {
...
public function read_file(){
      $contents = file_get_contents( 'D:\work-2015\promab/LEFT.txt');
      $product_arr = explode('http', $contents);
        unset($product_arr[0]);
        foreach ($product_arr as $k => $v)
        {
            $data[] = self::index1( 'http' . trim($v));
        }
        $export_cols = array( 'url' =>'url' ,'app'=>'app');
        export_excel( $export_cols , $data,'__-');
}

        //采集图片信息
        public function get_image_only( $url = "http://www.biovision.com/parp-antibody-clone-10h-4910.html")
        {
            $content = format_html($url);
            preg_match('#<noscript>(.*)</noscript>#iUs', $content,$preg_main );
            preg_match_all('#<img src="images\/(.*)"border="0"alt="(.*)"title="(.*)"width="230"height="150"hspace="5"vspace="5">#iUs', $preg_main[1],$preg_item );
            $product['url'] = $url;
            if( $preg_item )
            {
                if( count($preg_item[0]) )
                {
                    foreach ($preg_item[1] as $k=>$v )
                    {
                       $product['image_'.$k] = "http://www.biovision.com/images/images_big/".$v;
                    }
                }
            }
             return $product;
        }
        public function caiji_biovision_pit()
        {
           $con = file_get_contents( "C:\Users\li_hao\Desktop\biovision/antibody.txt");
           $con = str_replace(array("\n","\r","\r\n"),"|", $con);
           $arr = explode("|", $con);
           foreach ($arr as $k=>$v )
           {
               if($v){
                  $product[$k]= self::get_image_only(trim($v));
               }
           }
           $export_arr['url'] = 'url';
           for($i=0;$i<6;$i++)
           ...
           export_excel($export_arr,$product,'__antibody_bivision','C:\Users\li_hao\Desktop\biovision/');
        }
