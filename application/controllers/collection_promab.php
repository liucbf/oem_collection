...
class Collection_promab extends CI_Controller {
...
public function read_file(){
    ...
    public function index1( $url)
   {
            $content = format_html($url);
            $preg = '#<tr style="(.*)"><td>(.*)<\/td><td>(.*)<\/td><\/tr>#iUs';
            preg_match_all( $preg, $content, $arr );
             $product['url'] = $url;
            if(count($arr) && isset($arr[2]) )
            {
                $product['app'] = implode(',',$arr[2]);
            }
            return $product;
